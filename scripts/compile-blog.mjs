...
async function main() {
  ...
  const syncImportLines = posts.map(
    (post) => `import BlogPost_${post.slug.replace(/-/g, '_')} from './blog/${post.slug}'`,
  )
  const syncMapLines = posts.map(
    (post) => `  '${post.slug}': BlogPost_${post.slug.replace(/-/g, '_')},`,
  )
