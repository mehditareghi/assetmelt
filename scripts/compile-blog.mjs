...
const contentCache = new Map<string, ComponentType>()

const BLOG_POST_LOADERS: Record<string, () => Promise<{ default: ComponentType }>> = {}

export async function loadBlogPostContent(slug: string): Promise<ComponentType | undefined> {
  ...
  contentCache.set(slug, mod.default)
  return mod.default
}

export function getCachedBlogPostContent(slug: string): ComponentType | undefined {
...
async function main() {
  ...
  const loaderMapLines = posts.map(
    (post) => `  '${post.slug}': () => import('./blog/${post.slug}'),`,
  )
